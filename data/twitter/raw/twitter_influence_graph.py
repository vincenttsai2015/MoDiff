...
with open('twitter_social_graph.pkl','rb') as f:
...
action_log_files = ['reply.csv','mention.csv', 'retweet.csv']

print('Load action files...')
for action_file in action_log_files:  # 多個不同行為的csv
    behavior_type = action_file.split('.')[0]  # "reply", "mention", "retweet"
    df = pd.read_csv(action_file)

    for _, row in df.iterrows():
        user = row["user_id_1"]
        target = row["user_id_2"]        
...
print('Visualization')
# 設定顏色對應
behavior_colors = {"reply": "red", "mention": "green", "retweet": "blue"}
...
plt.savefig('twitter_influence.jpg')