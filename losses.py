...
def get_sde_loss_fn4_DT_c(sde_x, sde_adj, train=True, reduce_mean=False, continuous=True,
                    likelihood_weighting=False, eps=1e-5):
  reduce_op = torch.mean if reduce_mean else lambda *args, **kwargs: 0.5 * torch.sum(*args, **kwargs)

  def loss_fn(model_x, model_adj, x, adj, u, la, u_target):
    ...
    perturbed_eigen_diag = torch.diag_embed(perturbed_eigen).type(torch.complex64)
    u_conj_T = torch.conj(torch.transpose(u, -1, -2))
    perturbed_adj = torch.matmul(torch.matmul(u, perturbed_eigen_diag), u_conj_T)

    # u_conj_T = torch.conj(u).transpose(-1, -2)
    # perturbed_t = torch.bmm(u, perturbed_eigen_diag)
    # perturbed_adj = torch.bmm(perturbed_t, u_conj_T)
    ...
    score_x = score_fn_x(perturbed_x, perturbed_adj, flags, t, u_target, perturbed_eigen)
    # print("score_x:", score_x.shape)
    # score_adj = score_fn_adj(perturbed_x, perturbed_adj, flags, t, u, la)
    score_adj = score_fn_adj(perturbed_x, perturbed_adj, flags, t, 0, perturbed_eigen)
...
def get_sde_loss_fn4_DT_s(sde_x, sde_adj, train=True, reduce_mean=False, continuous=True,
                    likelihood_weighting=False, eps=1e-5):
  reduce_op = torch.mean if reduce_mean else lambda *args, **kwargs: 0.5 * torch.sum(*args, **kwargs)
  ...
  def loss_fn(model_x, model_adj, x, adj, u, la, u_target):
    ...
    nonzero_count_flag = torch.zeros((flags.shape[0]))
    eigen_mask = torch.zeros((la.shape)).to(adj.device)
    for i in range(flags.shape[0]):
      count = torch.count_nonzero(flags[i])
      nonzero_count_flag[i] = count
      eigen_mask[i, :count // 2] = 1
    ...
    #adj below are perturbed
    perturbed_eigen_diag = torch.diag_embed(perturbed_eigen).type(torch.complex64)
    u_conj_T = torch.conj(torch.transpose(u, -1, -2))
    adj = torch.matmul(torch.matmul(u, perturbed_eigen_diag), u_conj_T) #****
    ...
    # score_x = score_fn_x(perturbed_x, perturbed_adj, flags, t,u,la)
    score_x = score_fn_x(perturbed_x, adj, flags, t, u_target, perturbed_eigen) #*******
    # print("score_x:", score_x.shape)
    # score_adj = score_fn_adj(perturbed_x, perturbed_adj, flags, t, u, la)
    score_adj = score_fn_adj(perturbed_x, adj, flags, t, u_target, perturbed_eigen)  #*******
    score_adj_mean =  torch.mean(score_adj)
    # print("score_adj_mean:", score_adj_mean)
    # print("score_adj:", score_adj.shape)
    if not likelihood_weighting:
