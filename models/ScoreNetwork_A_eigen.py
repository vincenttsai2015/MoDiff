...
class ScoreNetworkA_eigen(torch.nn.Module):
    ...
    def __init__(self, max_feat_num, nhid, max_node_num, spec_dim, num_layers, num_linears,
        ...
        #                  use_bn=False, activate_func=F.elu)
        self.final_with_eigen = MLP(num_layers=2, input_dim=self.nfeat + spec_dim, hidden_dim=2 * spec_dim, output_dim=spec_dim,
                         use_bn=False, activate_func=F.elu)
...
class ScoreNetworkA_eigenU(torch.nn.Module):

    def __init__(self, max_feat_num, nhid, max_node_num, spec_dim, num_layers, num_linears,
                    c_init, c_hid, c_final, adim, depth=3, num_heads=4, conv='GCN'):

        super(ScoreNetworkA_eigenU, self).__init__()
