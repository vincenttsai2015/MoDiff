...
class DenseGCNConv(torch.nn.Module):
    ...
    def forward(self, x, adj, mask=None, add_loop=True):
        ...
        if add_loop:
            ...
            adj[:, idx, idx] = 1+0j if not self.improved else 2+0j

        out = torch.matmul(x, self.weight)

        adj_real = adj.real
        adj_imag = adj.imag
        deg_inv_sqrt_real = adj_real.sum(dim=-1).clamp(min=1).pow(-0.5)
        deg_inv_sqrt_imag = adj_imag.sum(dim=-1).clamp(min=1).pow(-0.5)
        deg_inv_sqrt = torch.complex(deg_inv_sqrt_real, deg_inv_sqrt_imag)

        #deg_inv_sqrt = adj.sum(dim=-1).clamp(min=1).pow(-0.5)

        adj = deg_inv_sqrt.unsqueeze(-1) * adj * deg_inv_sqrt.unsqueeze(-2)
        out = torch.matmul(adj.real, out)
