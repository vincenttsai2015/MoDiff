...
#
# 用法：
#   sbatch run_modiff.sh <dataset> <seed> [mode]
#
...
echo
echo "===== 資料形狀 ====="
python set_data_dims.py "$CFG" "$DATA_DIR" "$FILE1"
...
if [ "$MODE" = "eval" ]; then
    ...
    echo
    echo "===== TRAIN ====="
    python main.py --type train_comp --scale ${SCALE} \
...
echo
echo "===== SAMPLE + EVAL ====="
python main.py --type eval_comp --scale ${SCALE} \
