...
def data_log(logger, config):
    logger.log(f'[{config.data.data}]   init={config.data.init} ({config.data.max_feat_num})   seed={config.seed}   batch_size={config.data.batch_size} spectral_dim={config.data.spec_dim}')
    logger.log(f'File1: {config.data.file1}  File1: {config.data.file2}')
